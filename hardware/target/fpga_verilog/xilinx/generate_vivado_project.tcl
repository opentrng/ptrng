...
# $ vivado -mode tcl -source generate_vivado_project.tcl -tclargs arty_a7_100t xc7a100tfgg484-1 #
# ###############################################################################################
...
source "${hardware}/target/fpga_verilog/hdl_files.tcl"
lappend hdl_files_vhdl [list "work" [file normalize "${boardname}/target.vhd"]]
lappend hdl_files_vhdl [list "work" [file normalize "${hardware}/target/common/xilinx/vhdl/adc.vhd"]]
lappend hdl_files_vhdl [list "work" [file normalize "${hardware}/target/common/xilinx/vhdl/ring.vhd"]]
lappend hdl_files_vhdl [list "work" [file normalize "${hardware}/target/common/xilinx/vhdl/dpfifo.vhd"]]
...
foreach hdl_file $hdl_files_vhdl {	
	read_vhdl -library [lindex $hdl_file 0] -vhdl2008 [lindex $hdl_file 1]
}

# Link all Verilog into project
foreach hdl_file $hdl_files_verilog {	
	read_verilog -library [lindex $hdl_file 0] [lindex $hdl_file 1]
}
